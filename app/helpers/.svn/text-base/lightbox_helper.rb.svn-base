module LightboxHelper
  ...
  #  link_to_function(name, "Lightboxruby.showBoxString(" + tag("iframe",html_options_iframe.merge(:src => url)) + ")", html_options)
  #end
  
  def link_to_lightbox_image(name, image_url)
    link_to_function(name, "Lightboxruby.showBoxString('" + image_tag(image_url) + "');")
  end
  
  def link_to_lightbox(name, id, html_options = {})
    link_to_function name, "Lightboxruby.showBoxByID('#{id.to_s}')", html_options
  end
  ...
  def link_to_close_lightbox(name, html_options = {})
    link_to_function name, 'Lightboxruby.hideBox()', html_options
  end
  
  def button_to_close_lightbox(name, html_options = {})
    button_to_function name, 'Lightboxruby.hideBox()', html_options
  end 
...
private
  ...
  def lightbox_remote_options(remote_options, hidden_content_id)
    ...
    remote_options[:complete] = "Lightboxruby.showBoxByID('#{hidden_content_id}'); " + remote_options[:complete].to_s
    remote_options
  end
