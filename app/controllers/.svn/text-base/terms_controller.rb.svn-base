class TermsController < ApplicationController
  ...
  # GET /terms.xml
  def index
    @terms = Term.paginate(:all, :page => params[:page] || 1, :per_page => 7, :order => "term ASC")
  ...
  def create
    ...
    respond_to do |format|
      if @term.save
        flash[:notice] = "#{@term.term} was successfully created."
  ...
  def update
    ...
    respond_to do |format|
      if @term.update_attributes(params[:term])
        flash[:notice] = "#{@term.term} was successfully updated."
        format.html { redirect_to(:action => 'index') }
        format.xml  { head :ok }
      else
